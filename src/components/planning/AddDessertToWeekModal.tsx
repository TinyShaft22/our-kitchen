...
interface AddDessertToWeekModalProps {
  isOpen: boolean;
  onClose: () => void;
  meals: Meal[];
    ...
    <div className={depth > 0 ? 'ml-4' : ''}>
      ...
      {depth > 0 && (
        ...
        >
          ...
          {expanded ? (
            <FolderOpen className="h-4 w-4 text-honey" />
          ) : (
          ...
          {sortedMeals.map((meal) => (
            ...
            >
              {meal.imageUrl ? (
                ...
                <div className="w-10 h-10 rounded-soft bg-honey/10 flex items-center justify-center flex-shrink-0">
                  <span>🧁</span>
                </div>
              )}
              <span className="font-medium text-charcoal truncate">{meal.name}</span>
              <span className="text-honey text-sm ml-auto">→</span>
            </button>
          ))}
...
export function AddDessertToWeekModal({ isOpen, onClose, meals, onAdd }: AddDessertToWeekModalProps) {
  ...
  // Filter meals that are baking (desserts only)
  const bakingItems = useMemo(
    () => meals.filter((m) => m.isBaking),
    [meals]
  );

  // Build folder tree from baking items
  const folderTree = useMemo(
    () => buildFolderTree(bakingItems),
    [bakingItems]
  );
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to add dessert');
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        ...
        <DialogHeader className="sticky top-0 bg-cream border-b border-charcoal/10 px-4 py-3 flex flex-row items-center justify-between z-10 space-y-0">
          ...
          <DialogTitle className="text-lg font-semibold text-charcoal">
            {selectedMeal ? 'Set Servings' : 'Add Dessert'}
          </DialogTitle>
          {selectedMeal ? (
            <Button
              onClick={handleConfirm}
              disabled={saving}
              className="w-11 h-11 rounded-full bg-honey text-charcoal hover:bg-honey/90 disabled:opacity-50 p-0"
              aria-label="Confirm"
            >
              {saving ? (
          ...
          {selectedMeal ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 bg-white rounded-soft">
                {selectedMeal.imageUrl ? (
                  ...
                  <div className="w-16 h-16 rounded-soft bg-honey/20 flex items-center justify-center">
            ...
            <div className="space-y-4">
              ...
              {bakingItems.length === 0 ? (
                <EmptyState
                  icon="🧁"
                  title="No desserts yet"
                  description="Add baking recipes to your library first!"
                  variant="honey"
                />
              ) : searchResults !== null ? (
                    ...
                    {searchResults.map((meal) => (
                      ...
                      >
                        {meal.imageUrl ? (
                          ...
                          <div className="w-10 h-10 rounded-soft bg-honey/10 flex items-center justify-center">
                            <span>🧁</span>
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
